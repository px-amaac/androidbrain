...
function Controller(client, options) {
    ...
    //Origin Lat and Long
    this._origin = {lat:0.0, lon:0.0, heading:0.0};
    this._set_origin = true;
    // Register the listener on navdata for our control loop
    var self = this;
    client.on('navdata', function(d) {
        if (!this._busy && d.demo) {
            this._busy = true;
	    if(d.gps && self._set_origin){
		console.log("settingorigin" + d.gps.latitude + " " + d.gps.longitude);
	    	self._origin.lat = d.gps.latitude;
		self._origin.lon = -1*d.gps.longitude;
		self._origin.heading = d.demo.rotation.clockwise;
...
Controller.prototype.origin = function() {
    return this._origin;
}
...
//Give the controller the Latitude Longitude and heading and let it figure out where to go.
//You need the Magnometer calibrated for this.
Controller.prototype.gotolatlon = function(goal, callback) {
	var prose = {slat:0.0, slon:0.0, olat:0.0, olon:0.0, rotation_angle_degs:0.0};
	prose.slat = goal.lat;
	prose.slon = goal.lon;
	prose.olat = this._origin.lat;
	prose.olon = this._origin.lon;
	prose.rotation_angle_degs = this._origin.heading;
	var newgoal = EKF.lltoxy(prose);
	this.emit("localxy", newgoal);
	console.log("donewith lltoxy your at x= " + newgoal.x + " y= " + newgoal.y);
	if(this._follow && newgoal.x < 9.0 && newgoal.y < 9.0)
		return this.go(newgoal, callback)
}
