...
//loc is a data structure that contains tho following:
//olat, olon  origin lat and longitude
//xoffset_mtrs, yoffset_mtrs default ot 0
//rotation_angle_degs rotation in degrees east of north
//rms_error
EKF.lltoxy = function(loc) {
	with(Math){
		var xx, yy, r, ct, st, angle;
		angle = DEG_TO_RADIANS(loc.rotation_angle_degs);
		xx = (loc.slon - loc.olon)*METERS_DEGLON(loc.olat);
		yy = (loc.slat - loc.olat)*METERS_DEGLAT(loc.olat);
		r = sqrt(xx*xx + yy*yy);
		if(r)
		{
			ct = xx/r;
